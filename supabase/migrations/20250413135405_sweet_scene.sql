/*
  ...
  1. Changes
    ...
    - Add a new text-based example question
    - Create text_responses table for storing text-based responses
    - Set up appropriate RLS policies

  2. Security
    - Enable RLS on text_responses table
    - Add policies for users to manage their own responses
    - Add policies for admins to manage all responses
*/
...
-- Add a constraint to enforce unique display_order values (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'scenarios_display_order_unique'
  ) THEN
    ALTER TABLE scenarios 
...
-- Set up policies for text_responses using DO blocks to check existence first
DO $$
BEGIN
  -- Check if "Users can create text responses" policy exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_policy 
    WHERE polname = 'Users can create text responses' 
    AND polrelid = 'text_responses'::regclass
  ) THEN
...
DO $$
BEGIN
  -- Check if "Users can read own text responses" policy exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_policy 
    WHERE polname = 'Users can read own text responses' 
    AND polrelid = 'text_responses'::regclass
  ) THEN
...
DO $$
BEGIN
  -- Check if "Users can delete own text responses" policy exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_policy 
    WHERE polname = 'Users can delete own text responses' 
    AND polrelid = 'text_responses'::regclass
  ) THEN
...
DO $$
BEGIN
  -- Check if "Admins can manage all text responses" policy exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_policy 
    WHERE polname = 'Admins can manage all text responses' 
    AND polrelid = 'text_responses'::regclass
  ) THEN
